...
// ✔️ 그래서 위젯명 = 클래스명이라고 생각하시면 정확해요!
// Dart에서 정의된 클래스예요.

// 속성명은 클래스 생성자의 명명된(named) 파라미터를 지정할 때 사용하는 Dart 문법입니다.
// 만약 명명된 파라미터가 아닌 순서대로 넣는 파라미터면 그냥 순서대로 넣으면 되는데,
// Flutter는 대부분 **명명된 파라미터(named parameter)**를 사용해 가독성을 높여요.

// Scaffold → 클래스명 (위젯명)
// ( ... ) → 생성자 호출
// appBar: → 속성명 (named parameter)
// AppBar(...) → 또 다른 클래스 생성자 호출
// Text("본문") → 또 다른 클래스 생성자 호출

// ✅ 결론 요약
// 위젯명	= 클래스명
// ( )	클래스의 생성자 호출을 의미
// 속성명: 값	명명된 파라미터 전달 방식 (Dart 문법)
